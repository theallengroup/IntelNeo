...
class std extends db{ 
	...
	*/
	function load_i18n($mod,$dir='.'){
		...
		if(!isset($i18n[$mod])){
			...
			if(!file_exists($lang_file)){
				$this->i_error('no_strings_file','std041',array(
					'file'=>$mod,
					'shared'=>$lang_file,
	...
	*/
	...
	function field_is_foreign($field){
		if($field==''){
			$this->error("ERROR GRAVE field_is_foreign(): El campo: ".gp2($field).' está vacio!'.b2(),'std076');
		}
		if(!is_array($field)){
	...
	*/
	...
	function get_view_dataset($view){
		...
		$sql = $fs['sql'];
		return($q2 = $this->reduce_fs($this->q2obj($fs['sql']),$fs,$view['fields']));
	}
 ...
 * */	
	...
	function get_valid_modules($dir='.',$mode='controller'){
		return(array_merge($this->get_local_modules($dir,$mode),$this->get_shared_modules($dir,$mode)));
	}
	/**
	 * */
	...
	function ac_all_new2(){$this->ac_new2();}
	function ac_all_edit2(){$this->ac_edit2();}
	function ac_all_delete2(){$this->ac_delete2();}

	function ac_all_image_upload(){global $std_views,$i18n_std,$i18n;include(INCLUDE_DIR."actions/upload_image.php");}
	...
	function table2($a,$headers='none',$options=array()){
		$dx='';
		foreach($a as $row){
			$name = $row["name"];
			if(isset($row["icon"])){
			...
			$dx.='<div style="-moz-border-radius:10;border:0px solid black;text-align:center;padding:2px;margin:2px;width:130;height:120;padding-top:0;">';
			$dx.='<a class="standard_link" href="'.$row["url"].'" title='.$name.' >';
			$dx.=$this->iconize($icon,$name);
			$dx.='<br/>';
			#$row["name"] = str_replace("_","\n",$row["name"]);
			#$dx.=nl2br(chunk_split($row["name"],14,"<br/>"));
			if(strlen($row["name"])>12){
				$dx.=substr($row["name"],0,16).'...';
			}else{
				$dx.=$row["name"];
			}
			$dx.='</a></div></div>';
