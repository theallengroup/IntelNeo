...
/* damage*/
		...
		foreach($l as $k=>$row){
			...
			$dxx='';
			#wtf ? $me->make_link($mk,$got_headers[$k],'list_link',$me->fh($field['name']));
 ...
 */
		...
		//SEARCH FORM START PROCESS - ADD ITEMS TO SEARCH FORM
		if(count($final)==0){
			$no_items=1;
		}		
		...
		if($no_items==0 || "" != $_GET['__search_term']){
			...
			$t6=$this->get_simple_search_form($t6);
		}
		//END OF SEARCH FORM PROCESSING
		...
		*/
		...
		#OBTAIN THE MAIN FORM
		$form_struct=$t6->bare_fields();

		#OBTAIN LINKS AND SEARCH STUFF FROM THE MAIN FORM
		$search_box = $form_struct['visible_fields']['__search_term'];
		$links      = $form_struct['visible_fields']['links'];
		
		#p2($form_struct);
		#WHEN THERE IS NO RECORDS; DONT SHOW PAGINATION DATA
		...
		*/
		...
		$success=1;
		$envelope=1;
		$top_form_content = $form_struct["head"];
		$top_form_content.= "<table border=0 ><tr><td>".$search_box."</td><td>" .$form_struct["buttons"].'</td><td>'.$links.'</td></tr>';
		$top_form_content.="<tr><td colspan=3>".$message."</td></tr></table>";
		...
		if(count($final)==0){
			...
			$table_contents=$me->get_i_msg('no_items',array('name'=>$me->i18n('table_plural'),'ac'=>$ending));	
			$success=0;
			$envelope=0;
		}elseif($in_template==0){
			/** \todo 1005 allow CHECKBOX in templates (for now it's only in table())*/
			#old fashoned table()
			$table_contents=$this->table($final,$headers,
				array(//$o2["c"]
 ...
 [0] => Array
	...
	* */
	...
	function get_top_template(){
		$file_name=SHARED_MODULES_DIR.'templates/simple_search.html';
		if(file_exists('./template/simple_search.html')){
			$file_name='./template/simple_search.html';
		}
		$r=file_get_contents($file_name);//magic
		return($r);
	}
	/**
	...
	function get_simple_search_form($form_object){
		global $i18n_std;
		$me=&$this->caller;
		#$f=new form();
		...
		$form_object->add_field(array('name'=>'__search_term','type'=>'text','value'=>str_replace("\\",'',$this->remove_html($me->restore_value('__search_term')))));
		#$form_object->add_field(array('name'=>'mod','value'=>$me->program_name(),'type'=>'hidden'));
		$form_object->add_hidden_field('view_id',$me->current_view);
		...
		//.'&nbsp;&nbsp;'
		$form_object->add_field(array('name'=>'links','simple'=>$dx1,'type'=>'simple'));
		
		#$f->add_field(array('name'=>'ac','value'=>$_GET['ac'],'type'=>'hidden'));
		$form_object->add_submit_button(array('action'=>$_GET['ac'],'label'=>$me->get_i18n_text('search_ok')));
		//p2($f);
		return($form_object);
	}
